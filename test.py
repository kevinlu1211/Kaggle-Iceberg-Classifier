    ...
    try:
        ...
        for i in range(img.shape[2]):
            ...
            if opt_poly:
                ...
                # Add all of the combinations of Laplacian, Sobel, kurtosis and skewness
        ...
        if opt_poly:
            for x, y in combinations(st_interv, 2): # find all combinations of the basic stats for between each color channel
...
#Load data
train, train_bands = read_json('data/train.json')
test, test_bands = read_json('data/test.json')
...
for r, (train_index, test_index) in enumerate(kf.split(train_X, train_y)):
    ...
    params['max_depth'] = 4
    params['subsample'] = 0.9 # ratio of training examples to be used
    params['eval_metric'] = 'logloss'
    params['colsample_bytree'] = 0.8 # ratio of features to be used
    params['colsample_bylevel'] = 0.8 # ratio of features to be used in each split
